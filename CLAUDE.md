...
- `{书名}/对标/` — 对标作品分析
- `{书名}/仿写框架.md` — **仿写项目识别标记**（仅仿写项目存在此文件）

---

## 仿写项目自动识别

当用户说"继续""续写""日更"时，agent 自动检测项目根目录是否存在 `仿写框架.md`：
- **存在** → 仿写项目，加载框架作为质量锚点，路由到 `workflow-daily.md`
- **不存在** → 常规长篇项目，路由到 `workflow-daily.md`

用户无需手动指定"rewrite流程续写"，agent 自动完成识别和路由。
