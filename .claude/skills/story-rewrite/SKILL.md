...
```bash
python .claude/skills/inkos-rewrite/source_chapter_splitter.py split <源文.txt> novel-download-authors/{作者名}/{书名}/源文/
```
...
# 1. 拆章
python .claude/skills/inkos-rewrite/source_chapter_splitter.py split <源文.txt> novel-download-authors/{作者名}/{书名}/源文/
python .claude/skills/inkos-rewrite/source_chapter_splitter.py extract novel-download-authors/{作者名}/{书名}/源文/ <start> <end> <{书名}/源文_{x-y}.txt

# 2. 脚本先行：每章提取统计指纹（不耗token）
for i in range(start, end+1):
    python .claude/skills/inkos-rewrite/style_analyzer.py novel-download-authors/{作者名}/{书名}/源文/第{i}章.txt --output novel-download-authors/{作者名}/{书名}/蒸馏/mode-a/style_profile_{i}.json
...
```bash
# 1. 拆章
python scripts/source_chapter_splitter.py split <源文.txt> novel-download-authors/{作者名}/{书名}/源文/

# 2. 脚本先行：每章提取统计指纹（不耗token）
for i in range(1, total_chapters+1):
    python scripts/style_analyzer.py novel-download-authors/{作者名}/{书名}/源文/第{i}章.txt --output novel-download-authors/{作者名}/{书名}/蒸馏/mode-b/style_profile_{i}.json
...
2. 章纲：生成章纲（简化版，Mode B专用），保存到 {书名}/大纲/章纲_N.md
...
| 脚本 | 用途 | 位置 |
|------|------|------|
| `scripts/source_chapter_splitter.py` | 拆分源文章节 | `.claude/skills/story-rewrite/` |
| `scripts/style_analyzer.py` | 提取风格统计指纹 | `.claude/skills/story-rewrite/` |
| `scripts/validate_files.py` | 验证文件是否存在 | `.claude/skills/story-rewrite/` |
| `scripts/update_truth_files.py` | 更新真相文件 | `.claude/skills/story-rewrite/` |
| `tools/word_count.py` | 统计字数 | `.claude/skills/story-rewrite/` |
