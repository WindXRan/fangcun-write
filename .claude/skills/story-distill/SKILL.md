...
**定位**：Phase 7 之后、Phase 0 之前执行。关闭蒸馏闭环，让每次蒸馏产出反哺工具本身。

**核心理念**：蒸馏不是一次性提取，而是管道+工具的持续进化。每跑一个作者，extractor prompt、阈值表、模块库都应该比跑之前更完善。
...
| 缺口类型 | 检测方法 | 判定标准 |
|---------|---------|---------|
| **执行缺口** | 对比 `references/candidates/` 下实际文件数 vs 应有数（8个） | 缺文件 → 对应 extractor 未执行/失败 |
| **质量缺口** | 读取 Phase 6 `test-prompts.json` 的失败项，追溯根因 | 失败类型映射：A2不明确→extractor prompt 问题；阈值不准→阈值表问题；模块缺失→de-ai-modules 问题 |
| **收敛缺口** | 统计 Phase 3 `references/rejected/` 中因"信息不足"淘汰的规则数 | 淘汰数 > 阈值（建议初始值：心智模型≥2，其他≥3）→ Phase 1 精读不足 |

**质量缺口追溯矩阵**（压力测试失败 → 根因映射）：

| 压力测试失败模式 | 根因层 | 修正动作 |
|----------------|--------|---------|
| 应调用未调用 | extrator prompt 的 A2（触发场景）不精确 | 追加案例到 extractor.md |
| 不应调用却调用 | extrator prompt 的 B（边界）不清晰 | 补充边界说明到 extractor.md |
| 边界模糊判断错误 | 阈值表缺失该类场景 | 更新阈值表 |
| 去AI扫描漏检 | de-ai-module 覆盖不全 | 创建新 de-ai-module |
| 章纲结构不符合 | chapter-parser prompt 遗漏该模式 | 补充 metadata 示例到 chapter-parser.md |

### 8b：自动修正

根据 8a 检测结果，执行三类修正：

**1. Extractor prompt 补充**

在对应的 `extractors/{name}.md` 末尾追加：

```
## 已知遗漏案例（由 Phase 8 自动追加）

| 蒸馏作者 | 遗漏模式 | 补充案例 | 追加时间 |
|---------|---------|---------|---------|
| {作者名} | {检测到的遗漏} | {补入的规则/案例} | {日期} |
```

**规则**：不修改原 prompt 主体，只在末尾追加已知遗漏案例区块。保持向后兼容。

**2. 阈值表动态调整**

如果 Phase 3 跨书验证时，某条规则的频率数据使阈值明显偏移（如所有作者的 X 维度频率都偏低），则自动更新对应 extractor 的阈值说明。

**当前维护的阈值**（位于各 extractor prompt 内）：

| 阈值 | 所在文件 | 调整触发条件 |
|------|---------|-------------|
| 出现频率 ≥5%（非偶发） | 通用阈值（各 extractor 共享） | 连续3个作者该规则频率 >60% → 提升到10% |
| 独特性锚点频率 >60% | 通用阈值 | 连续3个作者该规则频率 <30% → 降至40% |
| 去AI模块匹配度阈值 | de-ai-extractor.md | 连续2个作者误匹配同一模块 → 降低匹配度 |

**3. 模块库补充**

如果 Phase 5.5 去AI策略发现某新特征（如空留的"句长标准差≥12字"规律）不在现有 de-ai-modules 覆盖范围内，则：
1. 创建新模块 `de-ai-modules/{feature-name}.md`
2. 注册到 extractors/de-ai-extractor.md 的模块候选列表

### 8c：演化日志

写入 `{作者名}/references/evolution-log.md`，记录本次蒸馏对工具链的贡献：
...
| 类型 | 内容 | 严重程度 | 修正方式 |
|------|------|---------|---------|
| 执行缺口 | extractor 8（去AI策略）未运行 | 高 | 自动重跑 |
| 质量缺口 | 规则「开篇不用背景铺垫」应力测试失败 | 中 | 补充边界说明 |
| 收敛缺口 | 心智模型"故事观"因信息不足淘汰 | 低 | 记录，下次精读补充 |

## 本次修正

| 修正对象 | 修改内容 |
|---------|---------|
| extractors/mental-model-extractor.md | 追加「已知遗漏案例」：{作者名}的{遗漏模式} |
| de-ai-modules/句长标准差.md | 新建模块，覆盖句长方差检测 |

## 跨蒸馏参考

- 下个作者建议重点提取：{建议维度}
- extractor prompt 置信度变化：{各 extractor 的本次调整汇总}
```
...
| 字段 | 说明 | 必填 |
|------|------|------|
| 蒸馏日期 | 格式 YYYY-MM-DD | 是 |
| 源书籍 | 本次蒸馏的原文数量+书名概览 | 是 |
| 检测到的缺口 | 每个缺口一行，标注严重程度 | 是 |
| 本次修正 | 每项修正一行，标注修改对象+内容 | 是 |
| 跨蒸馏参考 | 对下个作者的建议 | 推荐 |
...
| 情况 | 处理 |
|------|------|
| 本次无缺口 | 在 evolution-log.md 中标注「本次无缺口，工具链稳定」 |
| 自动修正失败 | 写入 evolution-log.md 的「待人工处理」section，标记对应项 |
| 阈值调整产生副作用 | 记录到 evolution-log.md，下个蒸馏回滚并重新调整 |
...
```
Phase 7 → Phase 8（检测→修正→记录）→ Phase 0（下个作者用改进后的工具）
               ↑                                      │
               └──────────────────────────────────────┘
```
