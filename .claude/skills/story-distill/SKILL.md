...
## Phase 2：并行提取（7个 subagent）

**借鉴**：cangjie-skill 的5个并行提取器 + nuwa-skill 的 prompt 模板

**目的**：从精读场景中提取决策框架

### Subagent 任务表

**并行** spawn 7 个 Task sub-agents（使用 Agent 工具，一次调用中发起 7 个）：
...
其他6个 subagent 按同样结构调整读取数据、提取内容、输出文件名。
...
1. 读取 `references/candidates/` 下所有7个文件
2. 对每条规则执行三重验证：
...
策略说明模块存放在 `de-ai-modules/` 目录下，每个模块是独立的 .md 文件：

| 模块文件 | 作用 |
|---------|------|
| `删连接词.md` | 去掉「然而」「但是」「因此」及单字连接词「但」「又」「却」「就」 |
| `拆长句.md` | >25字的句子拆成短句 |
| `降级副词.md` | 「十分」「非常」「微微」「轻轻」「缓缓」降级或替换 |
| `轻微倒装.md` | 语序微调，打破规律性 |
| `加微重复.md` | 模拟人类的重复习惯 |
| `加模糊词.md` | 加入「似乎」「好像」等模糊词 |
| `数字模糊化.md` | 精确数字改为模糊表达 |
| `污染AI流畅性.md` | 加入「卡顿感」 |
| `真实人类口误.md` | 加入微小错误（最后手段） |
| `风格迁移.md` | 按目标风格范例改写 |
| `降低ai预测概率.md` | 词性错配+量词反常态+半身截断，拉高文本熵值 |
| `精简字句.md` | 删除非剧情核心的静态描写和冗余字句 |
| `去ai味提示词升级版.md` | 综合方案：句子重组+半文半白+内容留白 |
| `标点后处理.md` | **必选，最后执行** |
| `得地的.md` | **必选，标点后处理之后执行** |

执行脚本（`.py`/`.ps1`）已迁至 `story-rewrite/tools/`，见 rewrite Phase 3 调用说明。

### 5.5a 预选（基础策略）

1. Phase 2 的 extractor 8（去AI策略提取器）自动分析作者习惯
2. 从模块库中选择 3-5 个匹配模块
3. 写入 SKILL.md 的「去AI策略」section
4. 产出：`extractors/de-ai-extractor.md` 的输出

### 5.5b 写后扫描（查漏补缺）

**时机**：narrative-writer 写完每章正文后，执行去AI模块前

**扫描器**：`extractors/post-write-scanner.md`
...
| 数字精确度 | ≥2次/章 | 数字模糊化 |
| 句式重复率 | 连续3句同结构 | 轻微倒装 |
| 流畅度异常 | 连续3句主谓宾无变化+句长标准差<5字 | 污染AI流畅性 |
| 情感表达单一化 | 重复情绪短语>3种 | 降级副词/加微重复 |
| 得地的覆盖率 | ≥1次/千字 | 得地的 |

**执行逻辑**：

1. 计算 8 个指标
2. 超标项对照预选模块 → 已覆盖则跳过，未覆盖则补充
3. 补充上限：2 个模块
4. 执行顺序：预选模块 → 补充模块（按优先级插入）→ 标点后处理 → 得地的
...
5.5a 预选模块
    ...
    └── 5.5b 写后扫描
            ...
            ├── 补充遗漏模块（上限 2 个）
            │
            └── 执行：预选模块 → 补充模块 → 标点后处理 → 得地的
```

### 执行

执行脚本（`.py`/`.ps1`）已迁至 `story-rewrite/tools/`，由 rewrite Phase 3 按策略配置调用。详见 rewrite SKILL.md Phase 3。
...
- 地(de) → 的（状语标志）
- 地(dì) → 不改（名词，如土地、地上）
- 单字连接词（但、又、却、就、也、还）→ 非对话行删除
- 数字精确 → 模糊化（非对话行，加"左右""来"）
- 标点规范化：分号/冒号/顿号 → 逗号、破折号 → 省略号

### 产出

- SKILL.md 的「去AI策略」section（含预选模块）
- 每章写后的扫描报告
...
```bash
python scripts/validate_output.py .claude/skills/story-style/{作者名}/SKILL.md
```
...
| `scripts/validate_output.py` | 输出验证（Phase 7 调用） |
