...
def split_paragraphs(content):
    ...
    # Insert newline after 。！？ when followed by Chinese char or quote
    content = re.sub(r'([。！？])\s*([\u4e00-\u9fff\u201c"])', r'\1\n\2', content)
    
    # Insert newline after closing quotes when followed by Chinese char
    content = re.sub(r'([\u201d"])\s*([\u4e00-\u9fff])', r'\1\n\2', content)
    ...
    for line in lines:
        ...
        if line:
            if line in ('……', '....'):
                cleaned.append('')  # Empty line for scene break
            else:
                cleaned.append(line)
