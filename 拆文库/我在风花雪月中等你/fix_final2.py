...
def split_paragraphs(content):
    """Split continuous text into paragraphs using simple string replacement."""
    # Step 1: Replace all "……" with newline markers
    content = content.replace('……', '\nBREAK\n')
    
    # Step 2: Insert newline after 。！？ when followed by Chinese char
    # Use a simple loop to handle this
    result = []
    i = 0
    while i < len(content):
        result.append(content[i])
        if content[i] in '。！？' and i + 1 < len(content):
            # Check if next char is Chinese or space+Chinese
            next_char = content[i+1]
            if next_char == ' ' and i + 2 < len(content) and '\u4e00' <= content[i+2] <= '\u9fff':
                result.append('\n')
            elif '\u4e00' <= next_char <= '\u9fff' or next_char == '\u201c':
                result.append('\n')
        # Insert newline after closing quotes
        if content[i] in '\u201d"' and i + 1 < len(content) and '\u4e00' <= content[i+1] <= '\u9fff':
            result.append('\n')
        i += 1
    
    content = ''.join(result)
    
    # Step 3: Replace BREAK markers with empty lines
    content = content.replace('\nBREAK\n', '\n\n')
