...
def split_paragraphs(content):
    ...
    # Insert newline before dialogue quotes (after non-quote, non-space)
    content = re.sub(r'([\u4e00-\u9fff。！？\u201d"])\s*([\u201c"])', r'\1\n\2', content)
    
    # Insert newline after closing quotes (before Chinese char)
    content = re.sub(r'([\u201d"])\s*([\u4e00-\u9fff])', r'\1\n\2', content)
    
    # Insert newline after 。！？ when followed by Chinese char
    content = re.sub(r'([。！？])\s*([\u4e00-\u9fff])', r'\1\n\2', content)
    ...
    # Insert newline before "说着"、"然后"、"这时" etc. (optional, for better readability)
    # content = re.sub(r'([\u4e00-\u9fff。！？])\s*(说着|然后|这时|之后|紧接着)', r'\1\n\2', content)
