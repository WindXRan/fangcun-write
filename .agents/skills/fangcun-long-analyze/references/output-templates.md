...
本文件按 fangcun-long-analyze 唯一管道的阶段编号组织：Stage 0-5 各阶段输出模板 + 停靠点「快速预览报告」模板 + 跨阶段通用速查。
...
## Stage 2 events.json 条目（chapter-extractor 输出）

extractor 按 `event_extraction.md` 的 12 字段 pipe 格式产出条目。主线程 split 后结构化写入 `events.json`。

格式见 `event_extraction.md`。

## Stage 2 角色过滤（同上，应用于章纲行出场角色列）
...
## Stage 2 情节点粒度（同上，应用于章纲行情节点列）

每章10-20个（密度公式：字数÷200 到 字数÷150）
...
> **Stage 4 直接按主题拆分输出多个文件**（与 `fangcun-import` 3.5 拆分规则、`fangcun-long-write` 项目结构对齐）。下游无需再 re-split。
...
输出 `拆文库/{书名}/文风.md`。整书级写作技法视图，供 fangcun-long-write 日更循环喂给 narrative-writer。
