...
Stage 3 聚合后必须生成两个权威产物，供 Stage 5 报告摘要和 `fangcun-long-write` 直接读取：
...
> 模板与示例见 [output-templates.md](output-templates.md) 「Stage 4 设定+关系」节。下游 `fangcun-import` 的 3.5 拆分步骤检测到 `设定/世界观/` 子目录已存在时直接 pass-through，不再 re-split。
