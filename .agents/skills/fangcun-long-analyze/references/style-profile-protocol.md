...
> **何时加载**：fangcun-long-analyze Stage 6 执行前。下游写作 skill 直接读 `文风.md`，不加载本协议。
...
- **读**：fangcun-import 同步到 `{项目}/对标/{书名}/文风.md`；fangcun-long-write 读项目对标视图（回退拆文库）
...
- `文风可用：否：{原因}` → 文风质量不足（例如原文缺失、锚点全是占位符）。fangcun-explorer 读取时返回 `gaps.profile_degenerate: true`，narrative-writer 跳过文风，按默认 Gates 写作，避免被误导。
- 兼容旧文件：若旧 `文风.md` 出现旧版内部降级标记，fangcun-explorer 也按“文风可用：否”处理。
