...
description: |
  ...
  触发方式：/fangcun-long-analyze、/长篇拆文、「帮我拆这本书」「拆这本书」「分析黄金三章」
...
├── 概要.md
├── events.json      # Stage 2 主产物（即章纲）：一章一行，12字段结构化（event管道+叙事骨架+情绪弧线+角色+关键信息），下游 Stage 3-6 全读
├── 章节/
│   ├── 第1章_深度拆解.md
│   ├── 第2章_深度拆解.md
│   └── 第3章_深度拆解.md   # 仅黄金三章保留深度拆解
├── 快速预览.md
├── 角色/
│   ├── {角色名}.md
│   └── 角色关系.md
...
这是 fangcun-long-analyze 唯一的执行管道。Stage 0-1 跑完后**自动停靠**产出快速预览报告（见下「Stage 1 停靠点」），用户确认后从 Stage 2 续跑。
...
| 2 | 逐章提取 → events.json | 分块章节文本 | **events.json（章纲）**：一章一个条目，12字段（event管道 + 核心事件/开头承接/结尾状态/衔接/情绪弧线/出场角色/关键信息）。**并行：每章 spawn chapter-extractor agent 产出一行 pipe，主线程 split 后结构化写入 events.json**。**计数验证：条目数 == 总章节数**。 | events.json 落盘 |
| 3 | 聚合分析 | **events.json**（情绪弧线+角色+关键信息） | 剧情/*.md + README.md + 故事线.md + **节奏.md + 情绪模块.md**。 | 质量检查通过 |
| 4 | 设定+关系（4a/4b/4c） | **4a**：events.json 情绪弧线+角色列（与 3 并行）；**4b/4c**：Stage 3 合并后角色数据+events.json | 设定/*.md + 角色/*.md。 | 4a/4b/4c 全部完成 |
...
| 6 | 文风 | 拆文报告.md + 章节/第1-3章_深度拆解.md + events.json + 原文/原文.txt | 文风.md（整书级写作技法视图：句长/标点/对话潜台词/情绪交替周期 + 4-6 段原文锚点范例片段，硬上限 ~4000 字。详见 [style-profile-protocol.md](references/style-profile-protocol.md) + [style-profile-generator.md](references/style-profile-generator.md)） | 文风落盘 `拆文库/{书名}/文风.md` |
...
3. **询问用户**（用 AskUserQuestion 风格的明确二选一）：
   ...
   - 选「就到这里」→ 管道结束，`_progress.md` 状态保持 `paused_after_stage1`，告知用户「之后可随时 `/fangcun-long-analyze` 同一本书，会自动从 Stage 2 续跑」。
...
- 主线程将 agent 输出的章纲行收集，按章号排序后写入 `章纲.md`
...
2. **质量失败**（输出落盘后跑 chapter-extractor.md「质量检查」自检，任一不达标——典型：情绪弧线缺失、基调标签超出枚举、角色名为昵称/通用称呼）→ **升级到 sonnet 重试 1 次**

**可机械校验的硬检查**（主线程落盘后直接检查）：
- 情绪弧线非空且基调词 ⊆ {紧张,轻松,悲伤,热血,爽,甜,温馨,恐怖,压抑,其他}
...
**最终落盘规则**：
- haiku 首次通过 → 条目加入 events.json，`_progress.md` 标记 `success`
...
- **agent 未部署**：agent 目录（优先 `.claude/agents/`，其次 `.opencode/agents/`）下的 `chapter-extractor.md` 不存在。`.claude/agents/` 通常不随仓库提交，由 `/fangcun-setup` 部署；模板源在 `skills/fangcun-setup/references/templates/agents/chapter-extractor.md`，必要时可手动复制部署。
- **环境不支持 spawn 子代理**：本 skill 正运行在某个子代理上下文中，无法再起下一层 agent。

### Stage 2 输出：events.json（章纲）

chapter-extractor agent 按 `event_extraction.md` 的 12 字段 pipe 格式输出每条事件。主线程 split 后结构化写入 `events.json`。

**events.json 条目格式**（前 7 字段为兼容旧 pipeline，后 5 字段为叙事骨架扩展）：

| # | 字段 | 示例 |
|---|------|------|
| 1 | 章节 | `第1章 面馆怒怼上司` |
| 2 | 涉及角色 | `林哲、杨雪` |
| 3 | 核心事件 | `林哲被羞辱后爆发怒骂，用违法证据反制上司` |
| 4 | 主线关系 | `强（主角人设建立）` |
| 5 | 信息密度 | `高` |
| 6 | 预估集长 | `50秒` |
| 7 | 情绪强度 | `冲突+转折` |
| 8 | 核心事件(短) | `怒怼上司反杀` |
| 9 | 开头承接 | `冷开：下班后在面馆偶遇上司` |
| 10 | 结尾状态 | `掌握上司违法证据，反客为主` |
| 11 | 衔接 | `→ 上司被迫谈条件` |
| 12 | 情绪弧线 | `压抑→爆发→轻松` |

**验证**：条目数 == 总章节数。不等 → 标失败章号。

events.json 是长期交付物，即全书章纲。人可通过 `python tools/events_to_table.py events.json` 查看表格视图。

events.json 下游使用：
- **Stage 3-4**：读情绪弧线+角色+关键信息做聚合
- **fangcun-write pipeline**：旧代码读 event 字段不变，新代码读叙事骨架字段做仿写
- **long-write 开书**：对标书 events.json 一览全书剧情
...
**流水线：** 长篇
**位置：** 拆文（长篇流水线第 2 步，在 fangcun-long-scan 之后、fangcun-long-write 之前）

| 时机 | 跳转到 | 命令 |
|---|---|---|
| 准备开写 | fangcun-long-write | `/fangcun-long-write` |
| 需要市场数据 | fangcun-long-scan | `/fangcun-long-scan` |
| 更适合短篇 | fangcun-short-scan → fangcun-short-analyze | `/fangcun-short-scan` |

> **选题决策回填**：若项目根有 `选题决策.md`（fangcun-long-scan 产出），拆完汇总报告（Stage 5 跑完）后会自动回填对应选题的"能爆的原因"（见上「Stage 5 后：选题决策回填」）。
