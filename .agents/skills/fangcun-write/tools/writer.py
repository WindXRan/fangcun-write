...
def _extract_character_brief(characters_full, ch_num):
    """从角色卡全文中提取本章相关角色的简要信息（控制 prompt 长度）"""
    if not characters_full or characters_full == "（无角色卡）":
        return "（无角色卡）"
    
    # 如果角色卡本身不长，直接返回
    if len(characters_full) < 2000:
        return characters_full
    
    # 提取主角团部分（通常在最前面，包含核心角色）
    lines = characters_full.split("\n")
    brief_lines = []
    ...
    for line in lines:
        ...
        if line.startswith("## "):
            ...
            if "主角" in line or "核心" in line:
                ...
                # 超过 2 个 section 后截断
                brief_lines.append(f"\n（...更多角色详见 characters.md，共 {len(lines)} 行）")
                break
...
def rewrite_chapter(config, ch_num, mode="imitation", reason=""):
    ...
    extra = {"reason": reason or "（无具体原因，整章重写）", "失败原因": reason or "（无具体原因，整章重写）"}
    variables = _build_variables(config, ch_num, mode, **extra)
