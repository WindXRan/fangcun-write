...
def step_relationship_extract(project_dir):
    print("\n[7/8] 提取角色关系图谱...")
    from tool_executor import run_tool
    r = run_tool("relationship-extract", {
        "user_input": "提取全本角色关系图谱",
    }, project_dir)
    print("  V" if "完成" in r else "  W " + r[:100])
    return True
...
def run_pipeline(book_name, author, source, channel="男频", project_dir=None):
    ...
    step_relationship_extract(out_dir)
