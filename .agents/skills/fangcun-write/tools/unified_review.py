...
def _parse_global_issues(llm_output, dimension):
    """解析全局维度 agent 的 XML 输出为 issue 列表。"""
    issues = []
    for m in re.finditer(r'<issue\s+priority="(P[012])"[^>]*>(.*?)</issue>', llm_output, re.DOTALL):
        prio = m.group(1)
...
<issues>
<issue priority="P0">问题描述</issue>
<issue priority="P1">问题描述</issue>
</issues>

没有问题则输出 <issues></issues>。
"""
...
def _parse_review_output(text):
    ...
    格式：
      <review>
        <chapter n="1">
          <score>80</score>
          <issues>
            <issue type="hook" severity="high" fix="修复建议">描述</issue>
          </issues>
        </chapter>
        <cross_issues>
          <issue chapters="1,2,3" type="continuity" severity="medium" fix="修复">描述</issue>
        </cross_issues>
      </review>
    ...
    review_text = review_match.group(1) if review_match else text

    # 解析每个 chapter 块
    for ch_match in re.finditer(r'<chapter\s+n="(\d+)">(.*?)</chapter>', review_text, re.DOTALL):
        ch_str = ch_match.group(1)
        ch_body = ch_match.group(2)

        score_m = re.search(r'<score>(\d+)</score>', ch_body)
        score = int(score_m.group(1)) if score_m else 50

        issues = []
        for issue_m in re.finditer(r'<issue\s+type="(\S+)"\s+severity="(\S+)"(?:\s+fix="([^"]*)")?[^>]*>(.*?)</issue>', ch_body, re.DOTALL):
            issues.append({
                "type": issue_m.group(1),
                "severity": issue_m.group(2),
                "desc": issue_m.group(4).strip(),
                "fix": issue_m.group(3) or "",
                "auto_fixable": False,
            })
        chapters[ch_str] = {"score": score, "issues": issues}

    # 解析跨章问题
    cross_match = re.search(r'<cross_issues>(.*?)</cross_issues>', review_text, re.DOTALL)
    if cross_match:
        for issue_m in re.finditer(r'<issue\s+chapters="([\d,]+)"\s+type="(\S+)"\s+severity="(\S+)"(?:\s+fix="([^"]*)")?[^>]*>(.*?)</issue>', cross_match.group(1), re.DOTALL):
            chs = [int(x.strip()) for x in issue_m.group(1).split(',') if x.strip()]
            cross.append({
                "chapters": chs,
                "type": issue_m.group(2),
                "severity": issue_m.group(3),
                "desc": issue_m.group(5).strip(),
                "fix": issue_m.group(4) or "",
            })
