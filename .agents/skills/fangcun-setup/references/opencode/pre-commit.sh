#!/bin/bash
### fangcun-hooks: BEGIN ###
# fangcun project commit validation
# Managed by fangcun-setup -- do not edit this block manually
# Checks for hardcoded character attributes in fangcun files (advisory only, never blocks)
(
set -euo pipefail
...
if [ -n "$WARNINGS" ]; then
  echo "=== fangcun Commit Warnings (advisory only, not blocking) ==="
  echo "$WARNINGS"
  echo "=== End Warnings ==="
fi
