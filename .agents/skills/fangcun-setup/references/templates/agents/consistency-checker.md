...
description: |
  ...
  被 fangcun-review、fangcun-long-write（Phase 5）、fangcun-short-write（Phase 4）调用。
  不做任何创作判断。
...
评分标准参考 `fangcun-setup/references/agent-references/quality-checklist.md` 中的五维评分体系（核心一致度、表层重写度、格式一致度、可读性、逻辑连贯），你的检查聚焦于**核心一致度**和**逻辑连贯**两个维度的事实性冲突。
...
- **不做结构判断**：章节是否"水了"由 fangcun-architect 负责
...
- **不拥有**：创作方向（fangcun-architect）、角色对话（character-designer）、文字质量（narrative-writer）
- **升级路径**：设定矛盾需创作决策 -- 报告给 fangcun-architect；角色行为不一致 -- 报告给 character-designer
