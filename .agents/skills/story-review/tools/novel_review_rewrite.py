...
def full_review_and_rewrite(config, start, end, batch_size=20, workers=5, max_rounds=3):
    ...
    # 备份旧报告到 _archive 子目录，然后清理
    import shutil
    archive_dir = Path(compare_dir) / "_archive"
    archive_dir.mkdir(exist_ok=True)
    archived = 0
    for pattern in ["*.md", "*.json", "*.txt"]:
        for old_file in Path(compare_dir).glob(pattern):
            if old_file.parent == archive_dir:
                continue
            shutil.move(str(old_file), str(archive_dir / old_file.name))
            archived += 1
    if archived:
        print(f"已备份 {archived} 个旧报告到 {archive_dir}")
