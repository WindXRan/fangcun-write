...
name: source-engine
description: |
  源书级分析引擎。提取事件表、生成故事骨架、制定改编策略。产物供 story-engine 和 drama-engine 共用。
...
```bash
# 全流程（事件→骨架→改编）
python .agents/skills/source-engine/tools/pipeline.py --config {config} --phase all
...
python .agents/skills/source-engine/tools/pipeline.py --config {config} --phase review       # 质量审核

# 查看状态
python .agents/skills/source-engine/tools/pipeline.py --config {config} --phase status
```
