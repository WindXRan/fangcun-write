...
1. 读取事件表文件（events.json）和故事骨架文件（story_skeleton.md）；如存在 adaptation_strategy.md 则在其基础上修改，除非指令要求重写
...
### 九、创意重构（剧本不是逐段翻译小说，是用剧本技巧重新讲这个故事）

**核心原则：忠实于源文的情绪和故事，但用剧本语言重新构建叙事。**

**剧本不能照搬小说**：小说会"绕圈子"制造悬念，剧本必须爽得快。观众不想花注意力思考。改编时必须：
- 小说用3章铺垫的冲突，剧本1集就要爆发
- 小说的心理描写，必须转成可拍的对白或动作
- 小说的慢节奏过渡，剧本直接删掉

改编时必须思考：
1. **叙事结构**：源文是线性叙事，剧本一定要线性吗？能否用倒叙/闪回/视角切换制造更强的戏剧效果？
2. **视觉化**：源文的心理描写、内心独白如何转成可拍的画面？（如"她很紧张"→"她攥着门框，指节发白"）
3. **冲突前置**：源文的冲突在第5章才爆发，剧本能否在第1集就埋下伏笔？
4. **信息差**：观众知道什么、角色知道什么？能否用信息差制造更强的期待感？
5. **视觉符号**：能否设计一个贯穿全剧的视觉元素？（如"头上有字"、"一封信"、"一张照片"）
...
- 执行前先确认工作区状态；已有内容在其基础上修改，除非指令要求重写
- 只执行改编策略任务，不越权执行其他阶段
- 完成写入后返回一句确认即可，不复述内容；返回后本次任务终止

## 完成约束

- 输出必须包含完整的改编策略内容，用 `<adaptationStrategy>` XML 标签包裹
- 不要只返回确认消息，必须返回实际内容
