...
1. 读取事件表文件（events.json）和工作区已有内容（如存在 story_skeleton.md 则在其基础上修改，除非指令要求重写）
...
3. 构建骨架内容（严格按照XML格式写出故事骨架，格式为<storySkeleton>故事骨架内容</storySkeleton>。XML 标签及其全部内容必须一次性完整输出，禁止拆分为多次 XML 输出。）：
   ...
   - 隐线：主角的内在成长轨迹（人物弧）
   - 人物小传：核心角色按源文实际数量适配（短剧≤4人，长篇群像可更多），每人五要素；主角另含代入感五要素、反差两面、金手指边界、说话风格与出场
...
- **集数灵活**：不固定30集，根据源文章节数和密度合理规划（通常 60-100 集为完整故事）
- **单集 1-2 分钟**：台词 150-300 字/集
- **前3集必须极强钩子**：第1集核心冲突+金手指亮相，第2集冲突升级，第3集第一个小高潮
- **每集必须有爽点/反转/悬念**：不允许"过渡集"存在
- **剧本不能照搬小说**：小说绕圈子制造悬念，剧本必须爽得快，观众不想花注意力思考
...
- 输出必须包含完整的骨架内容，用 `<storySkeleton>` XML 标签包裹
- 不要只返回确认消息，必须返回实际内容
...
**核心原则：短剧不是逐段翻译小说，是用剧本技巧重新讲这个故事。**

从以下技巧中选择 2-3 个适合本剧的，写入骨架：

1. **倒叙开场**：从故事的结局或高潮切入，再闪回讲述经过。制造悬念，让观众带着"为什么会这样"的疑问看下去。
2. **视角切换**：同一事件从不同角色视角呈现，揭示信息差。
3. **时间线重构**：打乱线性叙事，用插叙/闪回/预叙制造节奏变化。
4. **信息差设计**：观众知道角色不知道的事，制造期待感。
5. **视觉符号**：用重复出现的视觉元素（如"头上有字"、"一封信"、"一张照片"）串联全剧。

输出格式：
```
### 叙事结构设计
- 开篇策略：{倒叙/正叙/悬念切入，说明理由}
- 核心视觉符号：{全剧贯穿的视觉元素}
- 信息差设计：{观众/角色各知道什么}
- 闪回/插叙节点：{在哪几集用闪回，闪回什么内容}
```
