...
def call_api(api_key, model, user_prompt, max_tokens=4096):
    """调用 API 进行评分分析。"""
    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
    data = {
        "model": model,
        "messages": [
            {"role": "system", "content": "你是资深网文编辑，擅长对比分析文本质量、抄袭风险和风格匹配度。请严格按要求的JSON格式输出。"},
...
def auto_optimize(config, start, end, max_iterations=3):
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # 7. 备份旧章节，然后删除，重新运行pipeline
        print(f"  [3] 重新运行pipeline...")
        import shutil
        rewrites_dir = Path(config["rewrites_dir"])
        backup_dir = rewrites_dir / f"_backup_iter{iteration}"
        backup_dir.mkdir(exist_ok=True)

        chapters_dir = rewrites_dir / "chapters"
        if chapters_dir.exists():
            (backup_dir / "chapters").mkdir(exist_ok=True)
            for f in chapters_dir.glob("ch_*.txt"):
                shutil.copy2(f, backup_dir / "chapters" / f.name)
                f.unlink()
            print(f"    已备份 {len(list((backup_dir / 'chapters').glob('ch_*.txt')))} 章到 {backup_dir / 'chapters'}")

        guides_dir = rewrites_dir / "guides"
        if guides_dir.exists():
            (backup_dir / "guides").mkdir(exist_ok=True)
            for f in guides_dir.glob("plot_*.md"):
                shutil.copy2(f, backup_dir / "guides" / f.name)
                f.unlink()
            for f in guides_dir.glob("style_*.md"):
                shutil.copy2(f, backup_dir / "guides" / f.name)
                f.unlink()
            print(f"    已备份 guides 到 {backup_dir / 'guides'}")
