...
| 直接开写 | fangcun-write | `/fangcun-write` |
| 更适合短篇 | fangcun-short-scan | `/fangcun-short-scan` |

> **选题决策.md 交接**：Phase 4 产出的 `选题决策.md` 写在扫榜输出目录（扫榜常在没有小说项目时进行）。开书时把它搬到小说项目根目录，fangcun-write 开书阶段会自动读取；拆文（fangcun-long-analyze）会在汇总报告产出后回填对应选题的"能爆的原因"。
