        ...
        if not value:
            ...
            if var == 'FEISHU_APP_ID':
                ...
                if len(value) < 10:
                    print(f"❌ {var}: 长度不足")
                    print(f"   描述: {description}")
                    print(f"   期望长度: 至少10个字符")
                    results.append((var, False, "长度不足"))
                    all_valid = False
                else:
                    print(f"✅ {var}: 验证通过")
                    results.append((var, True, "验证通过"))
            elif var == 'FEISHU_SPREADSHEET_TOKEN':
                if not value.isalnum():
                    print(f"❌ {var}: 格式不正确")
                    print(f"   描述: {description}")
                    print(f"   期望格式: 字母数字组合")
...
def verify_all():
    ...
    verifications = [
        ("环境变量", verify_env_vars),
        (".env文件", verify_env_file),
        ("JSON配置文件", verify_json_config),
        ("飞书连接", verify_connection),
        ("数据文件", verify_data_files),
        ("依赖", verify_dependencies),
    ]
    ...
    for name, verify_func in verifications:
        try:
            if name == "环境变量":
                valid, details = verify_func()
                results.append((name, valid, details))
            else:
                valid, message = verify_func()
...
示例：
  python verify_feishu.py env         # 验证环境变量
  python verify_feishu.py connection  # 验证飞书连接
  python verify_feishu.py all         # 验证所有配置
""")
