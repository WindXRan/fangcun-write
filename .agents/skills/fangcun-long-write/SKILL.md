---
name: fangcun-long-write
version: 1.0.0
description: |
  长篇网文写作。从大纲到正文，辅助长篇网络小说的创作，包括世界观、人物、情节线管理。
  触发方式：/fangcun-long-write、/写长篇、「帮我开书」「写大纲」「日更」「续写」「继续写」「修改第X章」「回炉」「重写第X章」
metadata:
  openclaw:
    source: https://github.com/worldwonderer/oh-fangcun-claudecode
---

# fangcun-long-write：长篇网文写作
...
**先查选题决策**：如果项目根存在 `选题决策.md`（fangcun-long-scan Phase 4 产出，开书前搬入），读取它——取排在最前（可行性最高）的推荐选题作为开书起点，向用户确认：「扫榜建议写 X（能爆的原因 Y，差异化 Z），按这个开书？」并看 `扫榜日期`：距今较久则提示"市场数据可能过期，建议复扫"。用户认可 → 带该选题的题材/卖点/差异化进入 Phase 2。
...
3. 如均不存在，提示用户：「对标书原文已放入 `对标/{书名}/原文/`。要先用 `/fangcun-long-analyze` 拆解吗？拆完黄金三章会先给你预览，确认后可继续全量拆解，拆完后 `拆文报告.md` 会自动存入 `拆文库/{书名}/`，写作时会自动按 `对标/ → 拆文库/` 顺序读取。」
...
#### Agent 调用：fangcun-architect

fangcun-architect 属于高层级结构设计 agent。轻量题材定位优先由主会话完成；只有涉及复杂世界观、多线结构、强反转工程或用户明确要求时，才调用 fangcun-architect。确认选题方向后，如果项目已部署 fangcun-architect agent（检查 `.claude/agents/fangcun-architect.md` 是否存在），可 spawn `Agent(subagent_type: "fangcun-architect", prompt: "项目目录：{dir}\n任务类型：题材定位\n查询参数：{用户选择的方向+对标信息}")` 辅助题材分析和核心梗设计。如 agent 不可用，由主线程直接执行。
...
#### Agent 调用：fangcun-architect + character-designer

核心设定阶段，如果项目已部署对应 agent（优先检查 `.claude/agents/` 下的 `fangcun-architect.md` 和 `character-designer.md` 是否存在；不存在时再检查 `.opencode/agents/`），可 spawn 以下 agent 辅助：
- `Agent(subagent_type: "fangcun-architect", prompt: "项目目录：{dir}\n任务类型：核心设定\n查询参数：世界观构建+核心冲突设计")` — 辅助世界观和核心冲突设计
...
大纲搭建阶段优先由主会话产出卷纲+首批细纲；只有结构复杂、反转链多或主会话方案不稳定时，才调用 fangcun-architect agent。若项目已部署 fangcun-architect agent（检查 `.claude/agents/fangcun-architect.md` 是否存在），可 spawn `Agent(subagent_type: "fangcun-architect", prompt: "项目目录：{dir}\n任务类型：大纲搭建\n查询参数：卷级结构+细纲+钩子/反转/情绪弧线设计")` 辅助大纲排布、钩子/反转/情绪弧线设计。如 agent 不可用，由主线程直接执行。
...
│       ├── 设定/                  ← 从拆文库/结构化输出同步
│       │   ├── 世界观/             ← 按主题拆分到子目录（早期单文件版本由 fangcun-import 兜底转换）
│       │   │   ├── 背景设定.md
│       │   │   ├── 力量体系.md
│       │   │   ├── 地理.md
...
│   └── 上下文.md                  ← 正文级（日更进度摘要）
├── 参考资料/
│   └── {topic}.md             # fangcun-researcher 输出的研究资料
```
...
| 对标/{书名}/文风.md | 对标书 | analyze Stage 6 输出 → fangcun-import 同步 | Phase 4 每章写作前（文风召回） |
...
| 参考资料/{topic}.md | 按需 | Phase 4（fangcun-researcher 输出） | Phase 4 后续章节写作时复用 |
...
| 对标/{书名}/剧情/情绪模块.md | 对标书 | analyze Stage 3 输出 → fangcun-import 同步 | Phase 2 核心设定、Phase 3 大纲、Phase 4 每章写作前（读者需求 / 情绪引擎、可复现模块选择） |
| 对标/{书名}/剧情/节奏.md | 对标书 | analyze Stage 3 输出 → fangcun-import 同步 | Phase 3 大纲、Phase 4 每章写作前（关键信息推进、情绪触动点、爆发节奏参考） |
...
3. **`剧情/情绪模块.md` / `剧情/节奏.md` 缺失**：若对标书是 v12 新契约拆文库（`拆文报告.md` 已含读者需求/关键信息/节奏/可复现模块摘要，或导入报告未标 `legacy_deconstruction: true`），写前准备必须停下并提示重跑 `/fangcun-long-analyze` Stage 3+ 或重新 `/fangcun-import`，不得假装已召回权威模块。
4. **legacy 拆文库缺 `剧情/情绪模块.md`** → 写作继续；读者需求 / 情绪引擎与可复现模块依次回退到 `拆文报告.md` 对应摘要、`文风.md` 可借鉴技巧、匹配 `events.json` 同基调条目。记录 `legacy_deconstruction: true` + `module_missing`。
...
6. **有对标书但 `文风.md` 缺失** → 日更文风召回 fail-fast，提示先运行 `/fangcun-long-analyze` Stage 6 并 `/fangcun-import` 同步；**完全无对标项目**则跳过文风召回，不阻塞。
...
4. `events.json` 对应条目是具体章节证据，用来校验和补足权威索引
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果项目已部署 fangcun-explorer agent（优先检查 `.claude/agents/fangcun-explorer.md` 是否存在；不存在时再检查 `.opencode/agents/`），可 spawn `Agent(subagent_type: "fangcun-explorer", prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章")` 一次获取上下文）：
   ...
   - (7) `参考资料/{topic}.md`（如存在）— 历史研究资料（由 fangcun-researcher 产出）
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - 3.2 **模块召回与文风召回**：
     ...
     - (c) **文风召回**：按「对标书路径查找」规则读 `{对标书路径}/文风.md`（路径优先 `{项目}/对标/{书名}/`，回退 `拆文库/{书名}/`）；多本对标书时从 `设定/题材定位.md` 读 `主对标书` 字段。文风文件不存在 → **fail-fast 报错**：「对标书 X 缺少 文风.md。请用 `/fangcun-long-analyze` 跑 Stage 6 生成文风，再 `/fangcun-import` 同步。」不 inline 生成
     - (d) **匹配章节挑选**：从 `{对标书路径}/events.json` 中按本章目标情绪 grep 同基调条目，挑章 K 参考
...
| 时机 | 跳转到 | 命令 |
|---|---|---|
| 写完，去 AI 味 | fangcun-deslop | `/fangcun-deslop` |
| 想对比参考书 | fangcun-long-analyze | `/fangcun-long-analyze` |
| 需要市场方向 | fangcun-long-scan | `/fangcun-long-scan` |
| 太长，适合短篇 | fangcun-short-write | `/fangcun-short-write` |
