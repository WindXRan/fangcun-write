...
1. **△行代称**：△行使用"他/她/她们/他们"而非角色全名
2. **对话行括号**：对话行夹带（动作/神态/语气）描述，未拆到△行
3. **破折号滥用**：剧本正文中使用"——"作为连接符（剧情梗概除外）
4. **△行冒号感知**：△行用冒号引入角色主观感知，未拆成△动作+OS
5. **OS过长**：单段OS超过15字（应≤10字，情绪变化需加新动作+新OS）
6. **闪回用OS**：闪回画面中的旁白使用OS而非VO
7. **场景过多**：单集超过4个场景
8. **△行无视觉感**：△行只有干巴巴的动作描述，缺少镜头能拍到的画面细节

> 详细正反示例见 `prompts/format-rules.md`
> 自动校验工具：`tools/format_validator.py`
...
1. 读取故事骨架文件（story_skeleton.md）
2. 从【项目配置】读取：集数、单集时长、付费策略、章节范围
3. 读取事件表文件（events.json）
...
1. 读取改编策略文件（adaptation_strategy.md）和故事骨架文件（story_skeleton.md）
