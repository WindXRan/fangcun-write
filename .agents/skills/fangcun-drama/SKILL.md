...
跑完后 agent 应该：
1. **格式校验**：运行 `python .agents/skills/fangcun-drama/tools/format_validator.py {output_dir}/scripts`，自动检测格式违规（△代称、对话括号、破折号、OS长度、场景数等）
2. 读 `scripts/ep_001.txt`，检查格式是否正确（△标记、场景标题、台词）
3. **字数统计**：只计纯内容（△描述+对话+OS），不含剧情梗概、场景头、人物行、格式标记
4. 检查角色名是否正确
5. **场景价值判断**：对每个场景问"这个场景对观众有什么用？"，答不出来 = 废笔
6. **废笔检测**：删除过程展示、环境铺垫、无用对话、废话台词
7. 读 `reviews/` 目录，汇报审核评分
8. 如有问题，告诉用户

> 格式红线详见 `prompts/format-rules.md`，自动校验工具详见 `tools/format_validator.py`
