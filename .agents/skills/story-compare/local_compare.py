...
SYSTEM_PROMPT = """你是资深网文编辑，负责评估仿写作品的质量和给出优化建议。
...
5. **Prompt优化建议**：根据仿写成品暴露的问题，反推写作prompt应该如何调整。例如：如果仿写版AI痕迹重，建议prompt中加入"禁止使用首先/其次/然后等路标词"；如果台词雷同，建议prompt中加入"台词必须完全原创，不可照搬源文句式"。给出2-3条具体可执行的prompt修改建议。"""
...
## 差异分析
1. **叙事风格**：开篇方式、视角、基调、幽默来源有何不同？
2. **节奏控制**：信息密度、铺垫长度、高潮位置有何差异？
3. **人设塑造**：角色层次感、行为动机、互动细节有何区别？
4. **信息密度**：信息是否服务于情节和人物关系？
