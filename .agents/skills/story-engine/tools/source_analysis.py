...
产物存放在 projects/{作者}/{源书名}/_cache/ 下，供 story-engine 和 drama-engine 共用。
"""
...
def get_cache_dir(config):
    ...
    # drama-engine 兼容：从 source_dir 推断
...
# ─── 片段提取（供 guides/write 注入用）────────────────────────────────────

def get_chapter_event(config, ch_num) -> str:
    """从 events.json 提取指定章节的事件摘要（单行）。"""
    events = load_events(config)
    for e in events:
...
def get_skeleton_context(config, ch_num) -> str:
    """从 story_skeleton.md 提取与指定章节相关的片段（三幕归属 + 分集信息）。"""
    skeleton = load_skeleton(config)
    if not skeleton:
        return ""
...
def get_adaptation_principles(config) -> str:
    """从 adaptation_strategy.md 提取核心改编原则（前 5 条）。"""
