...
4. 流式分批（与开书并行，不阻塞）：
    - plot：10 agents 并行（优先，每agent 1章）
    - de-ai：plot 完成后启动，单脚本批量生成所有章节（零agent开销）
  ...
  开书 + plot + de-ai + 注入完成后 ──→ Phase 2 写章（直出，写章agent读取 plot_guide + de-ai_guide + 全书弧线 + 真相）
              ...
              下一批写章（读取最新真相，避免跨章矛盾）
                   ...
                   └── Phase 0 蒸馏（与开书完全并行，不阻塞）
                       ├── plot（流式到达，注入后再启动写章）
                        ├── de-ai（plot完成后批量脚本生成，写章阻塞等待，零agent）
...
加载 story-style skill。plot 优先，10 agents 并行（每agent 1章）。de-ai 在 plot 完成后单脚本批量生成所有 de-ai_guide（零agent开销）。写章需等 plot + de-ai + 开书 + 注入完成后才启动。
