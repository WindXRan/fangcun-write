...
description: |
  ...
  触发方式：/fangcun-review、/审查、「审查一下」「帮我审一下」
metadata:
  openclaw:
...
- `/fangcun-review` 或 `/fangcun-review full` → 优先 spawn 全部 4 个 Agent；如果当前已经在子代理内，核心 Agent 未部署/异常，或 spawn 失败，自动降级为 solo。
- `/fangcun-review lean` → 优先 spawn `fangcun-architect` + `consistency-checker`；如果当前已经在子代理内，任一所需 Agent 未部署/异常，或 spawn 失败，自动降级为 solo。
- `/fangcun-review solo` → 不 spawn Agent，由当前会话执行基础审查。
...
3. **检查核心 Agent 部署状态**（检查项目内 agents，同时兼容 Claude Code 和 OpenCode）：
   - 优先检查 `.claude/agents/`，其次检查 `.opencode/agents/`；两个目录任一存在即视为已部署
    - full 必需：`fangcun-architect.md`、`character-designer.md`、`narrative-writer.md`、`consistency-checker.md`
    - lean 必需：`fangcun-architect.md`、`consistency-checker.md`
    - 对每个必需 Agent 文件：
    ...
    - 如果 `.fangcun-deployed` 存在且 `agents_version` 缺失或小于 `14`，视为 stale deployment；不要 spawn，降级 `solo`，建议用户重新运行 `/fangcun-setup`。
   - 如果目标模式所需任一文件缺失或 malformed，**不要尝试 spawn 缺失/异常 Agent**；自动降级为 `solo`，并在报告开头写明：`Fallback: missing agents -> solo` 或 `Fallback: malformed agents -> solo`，列出问题文件，建议用户运行 `/fangcun-setup`。
...
`fangcun-review` 的核心审查标准必须始终可用。参考文件是增强资料，不是运行前提。
...
| 用途 | 规范路径 |
|---|---|
| 通用质量清单 | `fangcun-review/references/quality-checklist.md` |
| 通用内容评分 rubric | `fangcun-review/references/quality-rubric.md` |
| 去 AI 味方法 | `fangcun-review/references/anti-ai-writing.md` |
| 剧情循环/高潮公式 | `fangcun-review/references/plot-core-methods.md` |
| 角色关系/好感度 | `fangcun-review/references/character-relations.md` |
| 对话质量 | `fangcun-review/references/dialogue-mastery.md` |
| 审查禁用词 | `fangcun-review/references/banned-words.md` |
| 平台 rubric | `fangcun-review/references/rubrics/{fanqie,qidian,zhihu}.md` |
| 标点预检脚本 | `fangcun-review/scripts/normalize-punctuation.js` |
| AI句式预检脚本 | `fangcun-review/scripts/check-ai-patterns.js` |
...
full/lean 模式下，主会话必须把“审查基准包摘要”直接写进每个 Agent prompt。**不要要求子 Agent 必须读取 `fangcun-review/references/*` 才能完成任务**；子 Agent 可读取 `fangcun-setup/references/agent-references/*` 作为补充，但最终必须遵守本 skill 注入的 rubric 摘要和统一 Findings Schema。
...
4. **识别目标平台并加载 rubric**：
   ...
   - 番茄小说 → 优先读取 `fangcun-review/references/rubrics/fanqie.md`；不可读时使用内置番茄 fallback 摘要。
   - 起点 → 优先读取 `fangcun-review/references/rubrics/qidian.md`；不可读时使用内置起点 fallback 摘要。
   - 知乎盐言 → 优先读取 `fangcun-review/references/rubrics/zhihu.md`；不可读时使用内置知乎 fallback 摘要。
   - 未识别平台 → 优先读取 `fangcun-review/references/quality-rubric.md`；不可读时使用内置通用网文内容 rubric，并报告 `Rubric: generic web-fiction` 与 `Rubric Source: file | embedded fallback`。
...
6. **确定性预检（只报告，不修改）**：当审查范围包含本地正文文件路径时，运行本 skill 自带脚本：
   ...
   - `fangcun-review` 不修改文件；需要自动修复时建议转 `/fangcun-deslop`。
   - 默认 `--quote-mode keep`，不把知乎盐言短篇的 `「」` 当作问题；只有项目明确指定引号风格时才检查对应转换建议。
   - 这些脚本都是 `fangcun-review` 的本地副本，不引用其他 skill 的文件。

**Phase 1.5：可选 fangcun-explorer 预查询**。仅当 `Effective Mode` 仍为 `full`/`lean`、当前允许 spawn 且 Agent/Task 工具可用时，才可检查 agent 目录（优先 `.claude/agents/`，其次 `.opencode/agents/`）下的 `fangcun-explorer.md` 并 spawn `fangcun-explorer` 预查设定摘要；`solo` 或子代理递归保护场景下不得 spawn，只能直接 Read/Grep。Prompt 示例：
...
**Agent 1: fangcun-architect**（subagent_type: fangcun-architect）
- full/lean 均调用。
- 审查视角：主题对齐、大纲结构、钩子/反转质量、范围控制、平台期待。
- 提示指令：
  ```
  你是 fangcun-architect，从故事架构层面审查以下内容。
  ...
  Rubric Source: file | embedded fallback
  相关文件路径：{设定/大纲/细纲文件路径}
  可选补充参考：如项目已部署 fangcun-setup reference bundle，可读取 `fangcun-setup/references/agent-references/quality-checklist.md`、`fangcun-setup/references/agent-references/plot-core-methods.md`；若不可读，不影响审查。
  检查项：
  1. 这一章是否推进了故事主题？
  2. 大纲结构是否完整（钩子/爽点/悬念）？
  3. 情绪节奏是否合理？
...
- 提示指令：
  ...
  Rubric Source: file | embedded fallback
  相关角色文件：{角色设定文件路径}
  可选补充参考：如项目已部署 fangcun-setup reference bundle，可读取 `fangcun-setup/references/agent-references/character-relations.md`、`fangcun-setup/references/agent-references/dialogue-mastery.md`；若不可读，不影响审查。
  检查项：
  1. 角色语言风格是否与语言风格档案一致？
  2. 对话是否千篇一律或信息过满？
  3. 人物弧线是否连贯？
  4. 角色行为是否符合其动机？
  5. 对话是否有潜台词和信息控制？
  6. 爱情线好感度与 CP 行为是否匹配？（参照审查基准包摘要或可选 `fangcun-setup` 角色关系参考）
  7. 好感度进度是否可感知？
...
- 提示指令：
  ...
  可选补充参考：如项目已部署 fangcun-setup reference bundle，可读取 `fangcun-setup/references/agent-references/anti-ai-writing.md`、`fangcun-setup/references/agent-references/banned-words.md`、`fangcun-setup/references/agent-references/quality-checklist.md`；若不可读，不影响审查。
  检查项：
  1. 是否存在禁用词/套话/陈词滥调？
...
- 提示指令：
  ...
  审查基准包摘要：{Phase 1 形成的 rubric / fallback 摘要，必须内联}
  Rubric Source: file | embedded fallback
  可选补充参考：如项目已部署 fangcun-setup reference bundle，可读取 `fangcun-setup/references/agent-references/quality-checklist.md`；若不可读，不影响事实冲突扫描。
  检查项：
  1. 角色属性是否前后一致？
  2. 世界规则是否被违反？
...
3. **可选事实核查**：如果审查内容涉及需要验证的外部事实（历史年代、地理方位、职业细节等），只有在 `Effective Mode` 仍为 `full`/`lean`、当前不是子 Agent、Agent/Task 工具可用且 agent 目录（优先 `.claude/agents/`，其次 `.opencode/agents/`）下的 `fangcun-researcher.md` 已部署时，才可额外 spawn `fangcun-researcher` 搜索验证；`solo`、missing/malformed/stale/spawn failed 降级或子代理递归保护场景下不得 spawn，只能在报告中标记“需人工事实核查”。
...
- fangcun-architect: APPROVE / CONCERNS(n) / REJECT / NOT_RUN
...
lean 模式只 spawn `fangcun-architect` + `consistency-checker`。如果任一缺失，按 Phase 0 自动降级 solo。其余流程同 full。

---

## solo 模式

不 spawn Agent。先按 Phase 1 第 4 步识别目标平台并加载对应 rubric；即使是 solo，也必须用平台 rubric、`fangcun-review/references/quality-rubric.md` 或内置审查基准包校准判断。

solo 必须执行基础检查：
1. 格式合规性检查（戏剧单元/画面分段、无机械字数切分、无空行、对话格式、主语/角色名节奏）。
2. 简单的设定一致性 grep（角色名、属性、关键设定、伏笔关键词）+ 推理型一致性检查（规则边界、设定层级、跨章因果链、可滥用漏洞、代价一致性）。
3. AI 味与禁用词检查（优先读取 `fangcun-review/references/banned-words.md` 与 `fangcun-review/references/anti-ai-writing.md`，不可读时使用内置 AI 味 / 禁用词 fallback 速查）。
4. 通用网文内容评分（优先读取 `fangcun-review/references/quality-rubric.md`，不可读时使用内置通用网文内容 rubric）。
...
| 时机 | 跳转到 | 命令 |
|---|---|---|
| 要修改查出的问题 | fangcun-long-write / fangcun-short-write | 返回对应写作 skill 修改 |
| 发现 AI 味需清理 | fangcun-deslop | `/fangcun-deslop` |
| 需要重新拆解对标书 | fangcun-long-analyze / fangcun-short-analyze | `/fangcun-long-analyze` 或 `/fangcun-short-analyze` |
