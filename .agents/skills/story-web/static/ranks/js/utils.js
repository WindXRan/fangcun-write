...
function parseWordCount(wcStr) {
    if (!wcStr || wcStr === '未知') return 0;
    const s = wcStr.replace(/,/g, '');
    if (s.includes('万')) return parseFloat(s.replace('万字', '').replace('万', '')) * 10000;
    if (s.includes('字')) return parseFloat(s.replace('字', ''));
    return parseFloat(s) || 0;
}
