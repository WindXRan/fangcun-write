...
│   ├── 角色状态.md                ← 角色当前状态快照
│   └── 上下文.md                  ← 正文级（日更进度摘要）
├── 仿写框架.md                    ← 仿写质量锚点（从试水TXT开书时自动生成，可选）
├── 参考资料/
...
| 仿写框架.md | 全书 | /story-long-write 开书时从试水TXT+框架提取 | Phase 4 每章写作前（仿写质量锚点） |
...
当项目来自 `/story-rewrite` 试水（`仿写试水库/试水_*.txt` 存在）时，开书阶段额外执行：

1. 读取试水 TXT 中的「仿写框架」section，提取结构锚点、文风锚点、差异对照表、质量红线
2. 将提取内容写入 `仿写框架.md`（项目根目录）
3. 试水 TXT 中的 3 章正文作为参考样本保留，不重复写入 `正文/`

**此文件只在开书时生成一次**，后续日更自动加载，不再重复生成。

**仿写项目自动识别**：`仿写框架.md` 同时作为仿写项目的识别标记。日更 workflow（`workflow-daily.md`）在 Step 1 前会自动检测此文件是否存在：
- 存在 → 标记为仿写项目，加载框架作为质量锚点
- 不存在 → 常规日更模式

新开会话时，用户只需说"继续"或"续写"，agent 自动识别仿写项目并接入正确流程，无需手动指定。
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果项目已部署 story-explorer agent（检查 `.claude/agents/story-explorer.md` 是否存在），可 spawn `Agent(subagent_type: "story-explorer", prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章")` 一次获取上下文）：
   ...
   - (12) `仿写框架.md`（如存在）— 仿写质量锚点：节奏对齐表、文风锚点、差异对照表、质量红线
3. **准备层**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - 3.2 **模块召回与文风召回**：
     ...
     - (c) **模块召回**：从对标的结构化子目录（角色/剧情/设定）中按本章情节检索相关模块
      ...
      - (e) **仿写质量锚点**（如 `仿写框架.md` 存在）：读取框架中的「结构锚点」和「文风锚点」，将本章对应的节奏对齐表（情绪基调、转折点、场景序列）和文风约束（叙事视角、节奏特征、对话风格）纳入写作指令。与文风.md 冲突时，仿写框架的「质量红线」优先（如禁止升华句、禁止源文本角色名等硬约束）
