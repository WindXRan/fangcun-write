...
>
> **仿写质量锚点**：如项目根目录存在 `仿写框架.md`，每章写作前必须加载，将本章对应的节奏对齐表和文风约束纳入写作指令。仿写框架的「质量红线」（禁止升华句、禁止源文本角色名等）为硬约束，与文风.md 冲突时优先。
>
...
**每次日更开始时，必须先执行仿写项目检测**。检测逻辑：

1. 检查项目根目录是否存在 `仿写框架.md`
2. 如存在 → **直接跳转 story-rewrite 的断点续写流程**，不走下方 Step 1-2
3. 如不存在 → 常规日更模式，走下方 Step 1-4

**跳转指令**：检测到仿写项目后，按 `story-rewrite` SKILL.md 的「断点续写（从中断处继续）」执行：
1. 扫描 `正文/` 目录找到最大章节编号 N
2. 读取 `大纲/章纲_全书.md` 确认总章数
3. 检查追踪文件完整性
4. 从第 N+1 章开始，按 story-rewrite 的 Step 2（M×K 并行写作）续写

> **为什么不走 workflow-daily**：仿写项目的写作流程（章纲由 story-architect 生成、正文由 narrative-writer 写、M×K 并行、风格 injection）与 story-rewrite 完全一致，在 workflow-daily 里重复一套会降低质量且增加维护成本。直接复用 story-rewrite 的完整流程。

---

## Step 1：快速上下文加载（仅常规项目）
...
| 5 | `仿写框架.md` | 仿写项目识别标记 + 质量锚点 | 跳过（非仿写项目无此文件） |
...
一次加载多章细纲，**逐章串行 spawn narrative-writer 子agent 写正文**：不得把多章同时交给多个子代理并发写。长篇章节依赖上一章正文和追踪文件，并发会导致上下文断裂、追踪覆盖和标题去重失效。
...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **2.5 spawn narrative-writer 子agent 写正文**（⚠️ 必须用子agent写，主会话禁止直接写正文）：
     - 构造 prompt（见下方模板）
     - spawn `Agent(subagent_type: "narrative-writer", prompt: "...")`
     - 子agent 读取必要文件后直接写正文到 `正文/第XXX章_章名.md`
...
**narrative-writer 子agent prompt 模板**（日更场景）：
...
## ⚠️ 写作前检查清单（每章写之前必须核对，全部用 read 工具读取）
1. 读取设定锁：{书名}/设定/设定锁.md → 核对角色名、地名、人物关系
2. 读取本章细纲：{书名}/大纲/细纲_第{N}章.md → 了解本章事件、钩子、情绪目标
3. 读取上章正文：{书名}/正文/第{N-1}章_{章名}.md → 了解前情衔接
4. 读取语感样本：{书名}/设定/语感样本.md → 模仿写作风格
5. 读取节奏特征：{书名}/设定/节奏特征.md → 爽点分布+情绪弧线
6. 读取仿写框架：{书名}/仿写框架.md → 质量红线+文风约束（如存在，必须读取）

## 本次任务
写第{N}章，写入文件：{书名}/正文/第{N}章_{章名}.md

## 本章意图（从准备层输出）
{2.4 意图确认的一句话：情绪+节奏+文风指令}
...
> ⚠️ prompt 中所有路径都是相对于项目工作目录的。子agent 通过 read 工具读取文件，不假设内容。
> ⚠️ 子agent 写完后直接写文件，不返回正文到主会话。主会话通过 read 工具验证文件已写入。
...
5. **仿写一致性快检**（如 `仿写框架.md` 存在）：抽查本轮新写章节的章首2句，对照仿写框架的「质量红线」检查——是否出现源文本角色名/地名、是否有升华句/总结句、情绪基调是否与节奏对齐表一致。**必须执行**，不可跳过
